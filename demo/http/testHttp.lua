...
function getNetState()    
    log.info("sim.getStatus:",sim.getStatus())
    sim.setQueryNumber(true)
    log.info("sim.getNumber:",sim.getNumber())
    log.info("sim.getImsi:",sim.getImsi())
    log.info("sim.getId():",sim.sim.getId())

	log.info("getState:",net.getState())
	log.info("getNetMode:",net.getNetMode())

    -- sys.timerStart(getNetState,5000)--5s一次
    http.request("GET","https://www.baidu.com",nil,nil,nil,nil,cbFnc)
    http.request("GET","https://www.baidu.com",{caCert="ca.crt"},nil,nil,nil,cbFnc)
end
sys.timerLoopStart(getNetState,5000)

sys.taskInit(getNetState)