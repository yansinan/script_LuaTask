...
local function CameraCallBack(msg)
    log.info("CameraCallBack msg.msg_id,msg.result",msg.msg_id,msg.result)
	log.info("CameraCallBack CAMERA_SAVEHONE",disp.CAMERA_SAVEHONE,"CAMERA_CAPTURE:",disp.CAMERA_CAPTURE)
    if disp.CAMERA_CAPTURE == msg.msg_id then
        if msg.result then
            --设置照片保存路径
            --此处第二个参数表示阻塞或非阻塞，1非阻塞，0阻塞
            ret = disp.camerasavephoto("/testCamera.jpg",1)
            log.info("testCamera.takePhotoAndDisplay fileSize,ret",io.fileSize("/testCamera.jpg"),ret)
        	if ret ~= true then
				disp.camerapreviewclose()
				--关闭摄像头
				disp.cameraclose()		
    ...
    if disp.CAMERA_SAVEHONE == msg.msg_id or msg.msg_id==1 then
        if msg.result then
            --关闭摄像头预览
            disp.camerapreviewclose()
            --关闭摄像头
            ...
            --显示拍照图片
			log.info("CameraCallBack",WIDTH,HEIGHT,msg)
            if WIDTH~=0 and HEIGHT~=0 then
                disp.clear()
...
function takePhotoAndDisplay()
    ...
    --disp.camerapreviewxzoom(zoom)
    --缩小2倍
    log.info("takePhotoAndDisplay",disp.camerapreviewzoom(0xff))
	-- disp.camerapreviewzoom(-2)

    --如果有LCD，使用LCD的宽和高
    --如果无LCD，宽度设置为240像素，高度设置为320像素，240*320是Air268F支持的最大分辨率
    disp.camerapreview(0,0,320-128,0,WIDTH or DEFAULT_WIDTH,HEIGHT or DEFAULT_HEIGHT)
    log.info("takePhotoAndDisplay",disp.camerapreviewzoom(0xff))
    --设置照片的宽和高像素并且开始拍照
    --此处设置的宽和高和预览时的保持一致
    --此处的第三个参数表示拍摄质量，默认50，100最好，第四个参数表示阻塞或非阻塞，1非阻塞，0阻塞
    disp.cameracapture(WIDTH or DEFAULT_WIDTH,HEIGHT or DEFAULT_HEIGHT,50,1)
...
rtos.on(rtos.MSG_CAMERA,CameraCallBack)
