...
function ws:close(code, reason)
    ...
    -- 20220114 dr需要判断io.connected否则两次关闭会导致“response on nil socket”，并阻止重连
    if self.io.connected then self.io:close() end
...
function ws:start(keepAlive, proc, reconnTime)
    ...
    while true do
        ...
        self:close()
        log.info("websocket:Start", "与 websocket Server 的连接已断开!","等待重连",reconnTime)
        sys.wait(reconnTime)
    end
end
